...
public class Map extends AbstractMap {
    ...
    public function correctMapView(_arg_1:Camera):Point {
        ...
        if (Parameters.ssmode || stage.scaleMode == StageScaleMode.EXACT_FIT) {
            ...
            if (stage.scaleMode == StageScaleMode.NO_SCALE) {
                ...
                x = ((-(_local_2.x) * 800) / (SPC.sWidth / _local_7));
                y = ((-(_local_2.y) * 600) / (SPC.sHeight / _local_7));
            }
        }
    ...
    override public function calcVulnerables():void {
        ...
        for each (_local_1 in goDict_) {
            if (_local_1.props_.isEnemy_) {
                if (((!(_local_1.dead_)) && (!(_local_1.isInvincible)))) {
                    if (((_local_1.props_.isCube_) && (Parameters.data_.fameBlockCubes))) continue;
                    if (((!(_local_1.props_.isGod_)) && (Parameters.data_.fameBlockGodsOnly))) continue;
    ...
    override public function draw(_arg_1:Camera, _arg_2:int):void {
        ...
        if (stage.scaleMode == StageScaleMode.NO_SCALE) {
            x = ((-(_local_3.x) * 800) / SPC.sWidth);
            y = ((-(_local_3.y) * 600) / SPC.sHeight);
        }
        else {
            x = -(_local_3.x);
            y = -(_local_3.y);
        ...
        if (wasLastFrameGpu != Parameters.isGpuRender()) {
            if ((((wasLastFrameGpu == true) && (SPC.STAGE.stage3Ds[0].context3D)) && (!((SPC.STAGE.stage3Ds[0].context3D) && (!(SPC.STAGE.stage3Ds[0].context3D.driverInfo.toLowerCase().indexOf("disposed") == -1)))))) {
                SPC.STAGE.stage3Ds[0].context3D.clear();
                SPC.STAGE.stage3Ds[0].context3D.present();
            }
            else {
                map_.graphics.clear();
            }
