...
public class GameSprite extends AGameSprite {
    ...
    private function onEnterFrame(_arg_1:Event):void {
        ...
        var lastUpd:int = (timer - lastUpdate_);
        if (mui_.held) {
            mX = (SPC.STAGE.mouseX - mui_.heldX);
            Parameters.data_.cameraAngle = (mui_.heldAngle + (mX * (Math.PI / 180)));
            if (!Parameters.ssmode && Parameters.data_.tiltCam) {
                mY = (SPC.STAGE.mouseY - mui_.heldY);
                mui_.heldY = SPC.STAGE.mouseY;
                this.camera_.nonPPMatrix_.appendRotation(mY, Vector3D.X_AXIS, null);
            }
        }
        if ((timer - this.lastUpdateInteractiveTime) > 100) {
            this.lastUpdateInteractiveTime = timer;
            this.updateNearestInteractive();
        }
        this.map.update(timer, lastUpd);
        for each (var hit:Hit in this.hitQueue) {
            this.gsc_.playerHit(hit.bulletId, hit.objectId);
        }
        this.hitQueue.length = 0;
        this.camera_.update(lastUpd);
        if (!Parameters.ssmode) {
            if (((Parameters.timerActive) && (Parameters.data_.showTimers))) {
                if (this.timerCounter == null) {
                    this.addTimer();
                }
                if (timer >= Parameters.phaseChangeAt) {
            ...
            if (((this.enemyCounter) && (Parameters.data_.showEnemyCounter))) {
                this.enemyCounter.visible = true;
            }
        }
        else {
        ...
        var p:Player = map.player_;
        if (this.focus) {
            camera_.configureCamera(this.focus, ((Parameters.ssmode) && (p.isHallucinating)));
            map.draw(camera_, timer);
        }
        if (p) {
            if (Parameters.fameBot) {
                if (map.name_ == Map.NEXUS) {
                    if (Parameters.data_.famebotContinue == 2) {
                        ...
                        if (Parameters.fameWalkSleepStart == 0) {
                            Parameters.fameWalkSleepStart = timer;
                        }
                        if (p.hp_ < p.maxHP_) {
                            if ((timer - Parameters.fameWalkSleepStart) > Parameters.fameWalkSleep_toFountainOrHall) {
                                p.followPos.x = (nexusFountains.x + (Math.cos((timer << 4)) * 1.9));
                                p.followPos.y = (nexusFountains.y + Math.sin((timer << 2)));
                            }
                        }
                        else {
                            if (p.y_ > ((nexusRealms.y + 7) + (Math.sin((timer / 100)) * 3))) {
                                p.followPos.x = (nexusRealms.x + (Math.cos((timer / 100)) * 1.6));
                                p.followPos.y = (nexusRealms.y + (Math.sin((timer / 100)) * 2.6));
                            }
                            else {
                                if (Parameters.fameBotPortal) {
                                    distPort = PointUtil.distanceSquaredXY(p.x_, p.y_, Parameters.fameBotPortal.x_, Parameters.fameBotPortal.y_);
                                    if (((distPort <= 1) && (Parameters.fameBotPortal.active_))) {
                                        this.gsc_.usePortal(Parameters.fameBotPortalId);
                                    }
                                    else {
                                        if (distPort <= 100) {
                                            p.followPos.x = Parameters.fameBotPortalPoint.x;
                                            p.followPos.y = Parameters.fameBotPortalPoint.y;
                                        }
                                    }
                                }
                                else {
                                    if ((timer - Parameters.fameWalkSleepStart) > Parameters.fameWalkSleep_toFountainOrHall) {
                                        if (PointUtil.distanceSquaredXY(p.x_, p.y_, p.followPos.x, p.followPos.y) >= 29) {
                                            p.followPos.x = (nexusHallway.x + (Math.cos((timer << 4)) * 1.5));
                                            p.followPos.y = (nexusHallway.y + (Math.sin((timer << 2)) * 5));
                                        }
                                        else {
                                        ...
                                        if ((timer - Parameters.fameWalkSleepStart) > Parameters.fameWalkSleep_toRealms) {
                                            if (PointUtil.distanceSquaredXY(p.x_, p.y_, p.followPos.x, p.followPos.y) >= 1) {
                                                p.followPos.x = (nexusRealms.x + Math.cos((timer << 4)));
                                                p.followPos.y = (nexusRealms.y + Math.sin((timer << 2)));
                                            }
                                            else {
                                                Parameters.fameWalkSleep_toFountainOrHall = int.MAX_VALUE;
                                                p.textNotification("Pick a Realm!", 0xFF0000, 0);
                                            }
                                        }
                                    }
                                }
                        ...
                        if (((Parameters.data_.famebotContinue == 1) && (p.hp_ < p.maxHP_))) {
                            p.followPos.x = (nexusFountains.x + Math.cos((timer << 4)));
                            p.followPos.y = nexusFountains.y;
                        }
                        else {
                    ...
                    if (map.name_ == "Vault") {
                        if (p.hp_ < p.maxHP_) {
                            p.followPos.x = vaultFountain.x;
                            p.followPos.y = vaultFountain.y;
                        }
                        else {
                            if (Parameters.data_.famebotContinue == 2) {
                                if (Parameters.data_.disableNexus) {
                                    ...
                                    if (Parameters.fameWaitStartTime == 0) {
                                        Parameters.fameWaitStartTime = timer;
                                    }
                                    p.textNotification((("Waiting " + int((Parameters.fameWaitNTTime / 1000))) + ' seconds to "/nexustutorial"'), 3158271, 0);
                                    if ((timer - Parameters.fameWaitStartTime) > Parameters.fameWaitNTTime) {
                                        this.dispatchEvent(Parameters.reconRealm);
                        ...
                        if ((timer - lastCalcTime) >= 5) {
                            p.followPos = p.calcFollowPos();
                            lastCalcTime = timer;
                        }
                        if ((((timer - p.lastTpTime_) > Parameters.data_.fameTpCdTime) && (PointUtil.distanceSquaredXY(p.x_, p.y_, p.followPos.x, p.followPos.y) > Parameters.data_.teleDistance))) {
                            p.lastTpTime_ = timer;
                            p.teleToClosestPoint(p.followPos);
                        }
                    }
                }
            }
                ...
                if (Parameters.followPlayer) {
                    p.followPos.x = Parameters.followPlayer.x_;
                    p.followPos.y = Parameters.followPlayer.y_;
                }
            }
            if (this.creditDisplay_ != null) {
                this.creditDisplay_.draw(p.credits_, p.fame_, p.tokens_);
            }
            this.drawCharacterWindow.dispatch(p);
            if (this.evalIsNotInCombatMapArea()) {
                this.rankText_.draw(p.numStars_);
                this.guildText_.draw(p.guildName_, p.guildRank_);
            }
            if (p.isPaused) {
                if (Parameters.ssmode) {
            ...
            moveRecords_.addRecord(timer, p.x_, p.y_);
        }
        lastUpdate_ = timer;
    }
