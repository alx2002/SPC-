...
public class SavedCharactersList extends Event {
    ...
    public function SavedCharactersList(_arg_1:String) {
        ...
        this.parseVaultData(_local_2);
        if (!Parameters.data_.skipPopups) {
            this.parseTOSPopup();
        }
        this.reportUnlocked();
    ...
    private function parseVaultData(_arg_1:XML):void {
        var _local_3:String;
        var _local_6:Vector.<int>;
        var _local_4:Vector.<int>;
        var _local_7:int;
        var _local_5:int;
        if (_arg_1.hasOwnProperty("Vault")) {
            vaultItems = new Vector.<Vector.<int>>();
            for each (_local_3 in _arg_1.Vault.Chest) {
                _local_6 = new Vector.<int>();
                _local_4 = ParseUtils.parseIntList(_local_3);
                if (_local_4 != null) {
                    for each (var _local_2:int in _local_4) {
                        _local_6.push(_local_2);
                    }
                }
                if (_local_6.length != 8) {
                    _local_7 = (8 - _local_6.length);
                    _local_5 = 0;
