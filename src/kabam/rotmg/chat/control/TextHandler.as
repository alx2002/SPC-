    ...
    public function execute(_arg_1:Text):void {
        ...
        if (!Parameters.ssmode) {
            if (_arg_1.text_.indexOf("Current number of") != -1) {
                _local_3 = ("Enemies: " + _arg_1.text_.substring(40, (_arg_1.text_.length - 1)));
                this.hudModel.gameSprite.updateEnemyCounter(_local_3);
                return;
            }
        }
        ...
        if (_local_2) {
            ...
            if (Parameters.data_.mobNotifier && (_arg_1.text_.indexOf("server.dungeon_opened_by") != -1 || _arg_1.text_.indexOf("server.dungeon_unlocked_by") != -1)) {
                SoundCustom.play("ding");
            }
            if (Parameters.data_.AutoResponder) {
                switch (_arg_1.name_) {
                    case "#Thessal the Mermaid Goddess":
                        if (_arg_1.text_ == "Is King Alexander alive?") {
                            this.model.player.map_.gs_.gsc_.playerText("He lives and reigns and conquers the world");
                        }
                        break;
                    case "#Ghost of Skuld":
                        if (_arg_1.text_.indexOf("'READY'") != -1) {
                            this.model.player.map_.gs_.gsc_.playerText("ready");
                        }
                        break;
                    case "#Altar of Draconis":
                        if (_arg_1.text_ == "Choose the Dragon Soul you wish to commune with!") {
                            this.hudModel.gameSprite.gsc_.playerText("Black")
                        }
                        break;
                    case "#Craig, Intern of the Mad God":
                        if (_arg_1.text_.indexOf("say SKIP and") != -1) {
            ...
            if (Parameters.data_.mobNotifier) {
                switch (_arg_1.name_) {
                    case "#Oryx the Mad God":
                        if (_arg_1.text_ == '{"key":"server.oryx_closed_realm"}' || _arg_1.text_ == '{"key":"server.oryx_minions_failed"}') {
                            SoundCustom.play("treasure");
                        }
                        break;
                    case "#A Strange Presence":
                        if (_arg_1.text_.indexOf("Innocent souls") != -1) {
                            SoundCustom.play("treasure");
                        }
                        break;
                    case "#Horific Creation":
                        if (_arg_1.text_.indexOf("Me door is open") != -1) {
                            SoundCustom.play("treasure");
                        }
                        break;
                    case "#Secret mechanism":
                        if (_arg_1.text_.indexOf("The door to Daichi") != -1) {
                            SoundCustom.play("treasure");
                        }
                        break;
                    case "#Event Chest":
                        if (_arg_1.text_.indexOf("15 sec") != -1) {
