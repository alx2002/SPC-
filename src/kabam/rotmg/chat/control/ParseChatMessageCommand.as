...
public class ParseChatMessageCommand {
        ...
        switch (this.data.toLowerCase()) {
            case "/getcredentials":
                if (this.account.getUserId().indexOf("@") == -1) {
                    ...
                    this.addTextLine.dispatch(ChatMessage.make(Parameters.SERVER_CHAT_NAME, "You are not steam user."));
                }
                return (true);
            case "/l2m":
            case "/left":
            ...
            case "/constructs":
                ...
                if (Parameters.constructToggle) {
                    ...
                    this.addTextLine.dispatch(ChatMessage.make(Parameters.SERVER_CHAT_NAME, "Constructs Ignored"));
                }
                else {
                    MapUserInput.remIgnore(2309);
                    MapUserInput.remIgnore(2310);
                    MapUserInput.remIgnore(2311);
                    this.addTextLine.dispatch(ChatMessage.make(Parameters.SERVER_CHAT_NAME, "Constructs Unignored"));
                }
                Parameters.save();
                return (true);
            case "/bg":
            ...
            case "/world":
                ...
                return (true);
            case "/mscale":
                this.addTextLine.dispatch(ChatMessage.make(Parameters.SERVER_CHAT_NAME, "Map Scale: " + Parameters.data_.mscale));
                return (true);
            case "/fs":
            case "/fullscreen":
            ...
            case "/ao":
                ...
                this.addTextLine.dispatch(ChatMessage.make(Parameters.SERVER_CHAT_NAME, ("Alpha: " + Parameters.data_.alphaOnOthers)));
                return (true);
            case "/savemaptxt":
            ...
            default:
                ...
                command = this.data.match("^/automana (\\\\d+)$");
                if (command != null) {
                    Parameters.data_.autoMana = command[1];
                    Parameters.save();
                ...
                command = this.data.match("^/psize (\\d+)");
                if (command != null) {
                    Parameters.data_.playerSize = parseInt(command[1]);
                    this.hudModel.gameSprite.map.player_.textNotification("Set Size to: " + command[1]);
                    Parameters.save();
                    return (true);
                }
                ...
                /*command = this.data.match("^/moveto (\\d+) (\\d+)");
                if (command != null) {
                    player.moveTo(command[1], command[2]);
                    return (true);
                }*/
                ...
                if (command != null) {
                    SPC.STAGE.frameRate = command[1];
                    this.addTextLine.dispatch(ChatMessage.make(Parameters.SERVER_CHAT_NAME, ("Framerate set to " + SPC.STAGE.frameRate)));
                    return (true);
                }
                ...
                command = (((Parameters.data_.replaceCon) ? (this.data.toLowerCase().match("^/conn ?(\\w*) ?(\\w*) ?(\\w*)")) : (this.data.toLowerCase().match("^/con ?(\\w*) ?(\\w*) ?(\\w*)"))));
                if (command != null) {
                    var realm:Array;
                    var isRealm:Boolean;
    ...
    private function listCommands():Boolean {
        ...
        if (command != null) {
            ...
            this.addTextLine.dispatch(ChatMessage.make(Parameters.HELP_CHAT_NAME, ("Adding to spam text: " + command[1])));
            return (true);
        }
        ...
        if (command != null) {
            ...
            this.addTextLine.dispatch(ChatMessage.make(Parameters.HELP_CHAT_NAME, "Spam text cleared"));
            return (true);
        }
        ...
        command = this.data.toLowerCase().match("^/suicide");
        if (command != null) {
            Parameters.suicideMode = (!(Parameters.suicideMode));
            this.hudModel.gameSprite.map.player_.textNotification(((Parameters.suicideMode) ? "Suicide On" : "Suicide Off"), 0xA00000);
            return (true)
        }
