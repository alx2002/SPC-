...
public class PetFeedProgressBar extends UIGridElement {
    ...
    public function PetFeedProgressBar(_arg_1:int, _arg_2:int, _arg_3:String, _arg_4:int, _arg_5:int, _arg_6:int, _arg_7:int, _arg_8:uint, _arg_9:uint, _arg_10:uint = 0) {
        ...
        var _local_11:* = _arg_6;
        this._currentLevel = _local_11;
        this._previousLevel = _local_11;
    ...
    private function createBarShapes():void {
        ...
        this._maxShape.graphics.beginFill(MAX_COLOR, 1);
    ...
    private function render(_arg_1:int, _arg_2:int):void {
        ...
        var _local_8:int = (this._maxValue - AbilitiesUtil.abilityPowerToMinPoints(this._currentLevel));
        var _local_9:int = (_arg_1 - AbilitiesUtil.abilityPowerToMinPoints(this._currentLevel));
        var _local_5:Number = ((this._currentLevel == this._maxLevel) ? 1 : (_local_9 / _local_8));
        if (this._animateLevelProgress) {
            this.setLevelLabel(this._previousLevel);
            this.animateToCurrentLevel();
        }
        else {
            this.drawProgress(_local_5);
        }
        if (_local_3) {
            if (_arg_2 >= this._maxValue) {
                _local_12 = AbilitiesUtil.abilityPointsToLevel(_arg_2);
                if (_local_12 >= this._maxLevel) {
                    _local_12 = this._maxLevel;
                    this.drawSimulatedProgress(this._componentWidth, MAX_COLOR);
                    this.setLevelLabel(_local_12);
                }
                else {
                    _local_11 = AbilitiesUtil.abilityPowerToMinPoints((_local_12 + 1));
                    _local_7 = AbilitiesUtil.abilityPowerToMinPoints(_local_12);
                    _local_10 = (_local_11 - _local_7);
                    _local_6 = (_arg_2 - _local_7);
                    this.drawSimulatedProgress((_local_6 / _local_10));
                    this.setLevelLabel(_local_12);
                }
            }
            else {
                _local_6 = (_arg_2 - AbilitiesUtil.abilityPowerToMinPoints(this._currentLevel));
                _local_4 = (_local_6 / _local_8);
                this.drawSimulatedProgress(_local_4, _local_5);
            }
        }
    ...
    private function setLevelLabel(_arg_1:int):void {
        ...
        if (_arg_1 > this._currentLevel) {
            DefaultLabelFormat.petStatLabelRight(this._levelLabel, MAX_COLOR);
        }
        else {
